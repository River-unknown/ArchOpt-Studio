...
// Memory helpers (put/get double/int) remain the same...
...
// Node management needs to account for sizeof(Node) with padding
#define NODE_STRUCT_SIZE 32 // Correct size for this struct with padding
...
void put_node(unsigned int node_addr, Node* node) {
    put_int(node_addr, node->feature_index);
    put_double(node_addr + 8, node->threshold); // Note the +8 offset due to padding
    put_int(node_addr + 16, node->predicted_class);
    put_int(node_addr + 20, node->left_node_addr);
    put_int(node_addr + 24, node->right_node_addr);
}
Node get_node(unsigned int node_addr) {
    Node node;
    node.feature_index = get_int(node_addr);
    node.threshold = get_double(node_addr + 8);
    node.predicted_class = get_int(node_addr + 16);
    node.left_node_addr = get_int(node_addr + 20);
    node.right_node_addr = get_int(node_addr + 24);
    return node;
}
...
// NEW: Select a random subset of features
...
unsigned int build_tree(int* samples, int n_samples, int depth) {
    if (depth >= MAX_DEPTH || n_samples < MIN_SAMPLES_SPLIT) {
        ...
        Node leaf_node = {-1, 0.0, majority_vote(samples, n_samples), 0, 0};
    ...
    Node current_node = {best_feature, best_threshold, -1, 0, 0};
...
// Setup and evaluation functions (read_csv, split_data, main etc.) are the same...
void read_csv_sim() {
...
int main() {
    ...
    for (int i = 0; i < MAX_ROWS; i++) {
        for (int j = 0; j <= NUM_FEATURES; j++) {
            addr_data[i][j] = current_addr; current_addr += 8;
        }
    ...
    read_csv_sim();
    split_data_sim();
    printf("Growing the Random Forest (AoS Layout)...\n");
    RandomForest rf;
    grow_forest(&rf);
