...
typedef struct Node {
    int feature_index;
    double threshold;
    int predicted_class; // -1 if not a leaf node
...
/**
 * @brief Writes a Node struct to the simulated memory pool.
 */
void put_node(unsigned int node_addr, Node* node) {
    unsigned int base_addr = node_addr;
    put_int(base_addr, node->feature_index);
    put_double(base_addr + 4, node->threshold);
    put_int(base_addr + 12, node->predicted_class);
    put_int(base_addr + 16, node->left_node_addr);
    put_int(base_addr + 20, node->right_node_addr);
}

/**
 * @brief Reads a Node struct from the simulated memory pool.
 */
Node get_node(unsigned int node_addr) {
    Node node;
    unsigned int base_addr = node_addr;
    node.feature_index = get_int(base_addr);
    node.threshold = get_double(base_addr + 4);
    node.predicted_class = get_int(base_addr + 12);
...
unsigned int allocate_node() {
    ...
    unsigned int node_addr = addr_node_pool + (next_node_idx * sizeof(Node));
    next_node_idx++;
    return node_addr;
}
